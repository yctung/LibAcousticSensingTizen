...
static bool _connect_sensing_server() {
	libdata_s *ld = &sld;

	DLOG_PRINT_DEBUG_MSG("_connect_sensing_server, server = %s:%s", ld->server_addr, ld->server_port);

	// open a client socket to connect
	struct sockaddr_in server_addr;
	struct hostent *server;
	ld->sockfd = socket(AF_INET, SOCK_STREAM, 0);
	server = gethostbyname(ld->server_addr);
	if (server == NULL) {
...
bool libas_is_ready_for_sensing() {
	libdata_s *ld = &sld;
	if (ld->is_remote_mode && !ld->is_connected) {
		return false;
	}
...
void libas_init_as_remote_mode(char *addr, char *port) {
	DLOG_PRINT_DEBUG_MSG("libas_init_as_remote_mode");

	libdata_s *ld = &sld;
	ld->is_remote_mode = true;
	sprintf(ld->server_addr, "%s", addr);
	sprintf(ld->server_port, "%s", port);
	...
	_audio_init();
	if (ld->is_remote_mode) {
		ld->is_connected = _connect_sensing_server();
	}
}
